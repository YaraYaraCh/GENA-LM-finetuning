...
@torch.no_grad()
def evaluate(model, dataloader, device, seq_len, writer, global_step, split='valid'):
    model.eval()
    ...
    for k, v in metrics.items():
        writer.add_scalar(f'{split}/{k}', v, global_step)

    logger.info(f'[{split}] step={global_step} | ' + ' | '.join(f'{k}: {v:.4f}' for k, v in metrics.items()))
    model.train()
    return metrics
...
def main():
    ...
    # ── Папка для сохранения и TensorBoard ────────────────────────────────
    model_path = Path(args.model_path)
    model_path.mkdir(parents=True, exist_ok=True)
    tb_dir = model_path / 'tensorboard'
    ...
    while global_step < args.iters:
        ...
        if global_step % args.valid_interval == 0:
            metrics = evaluate(model, valid_loader, device, args.input_seq_len,
                               writer, global_step, split='valid')
    ...
    evaluate(model, valid_loader, device, args.input_seq_len, writer, global_step, split='valid_final')

    logger.info('=== Test set ===')
    evaluate(model, test_loader,  device, args.input_seq_len, writer, global_step, split='test')
